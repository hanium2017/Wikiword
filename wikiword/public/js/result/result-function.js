...
function moreAjax (object, handleData) {
  ...
  axios.post(`http://${host}:3000/more`, { title: object.title })
        .then((response) => {
          ...
          handleData(object, moreContentRender)
        })
}
...
function getJSONData (object, callback) {
  axios.get(`http://${host}:${object.port}/${object.title}?search=${data}&pageNum=${object.pageNum}`)
        .then((response) => { callback(object.title, response.data) })
...
function contentRender (title, items) {
  ...
  if (title !== 'wikipedia') { moreBtnRender(moreBtnElement, title, items) }
...
function moreBtnRender (moreBtnElement, title, items) {
  (title === 'youtube') ? moreBtnElement.setAttribute('nextNum', items.pop())
